 ...
 //        if (DEBUG) out.println ("put: load factor = " + lf);
      ...
      */
      ...
      private void split() {
        ...
        hTable.set(isplit, new Bucket()); // Reset the bucket at isplit
        hTable.add(new Bucket()); // Add a new bucket at the end
        isplit++;
    
        // Redistribute entries
        for (var entry : splitMap.entrySet()) {
            var index = h(entry.getKey()); // Re-hash the key
            if(index < isplit) index = h2(entry.getKey());
            var bucket = hTable.get(index);
            if (bucket.nKeys < SLOTS) {
                bucket.add(entry.getKey(), entry.getValue());
            } // Typically, you'd handle overflow here
        }
